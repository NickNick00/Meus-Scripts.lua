...
local ITEM_DATABASE = {
    -- Combustivel
    Log = "Combustivel",
    Wood = "Combustivel", -- Adicionado para caso o item se chame "Wood"
    Chair = "Combustivel",
    Biofuel = "Combustivel",
    Coal = "Combustivel",
    ["Fuel Canister"] = "Combustivel",
    ["Oil Barrel"] = "Combustivel",
    Sapling = "Combustivel", -- Assumindo que "Mudas" é "Sapling" no jogo

    -- Comida e Cura
    Carrot = "Comida e Cura", Corn = "Comida e Cura", Pumpkin = "Comida e Cura", Berry = "Comida e Cura", Apple = "Comida e Cura", -- Maçã
    ...
    ["Carrot Cake"] = "Comida e Cura", ["Jar o' Jelly"] = "Comida e Cura",
    Crab = "Comida e Cura", -- Caranguejo
    Salmon = "Comida e Cura", -- Salmão
    Swordfish = "Comida e Cura",
    Fruit = "Comida e Cura", -- Fruta
    Spice = "Comida e Cura", -- Tempero
    ["Dinner Meat"] = "Comida e Cura", -- Janta de Carne
    Bandage = "Comida e Cura", Medkit = "Comida e Cura", Pepper = "Comida e Cura", -- Pimenta

    -- Sucata (Engrenagens)
    Bolt = "Sucata", -- Parafuso (nome mais provável no jogo)
    Screw = "Sucata", -- Outro nome comum para parafuso
    ["Sheet Metal"] = "Sucata", -- Chapa Metálica
    ["UFO Junk"] = "Sucata", ["UFO Component"] = "Sucata", ["UFO Scrap"] = "Sucata", -- Entulho/Componente/Fragmentos de OVNI
    ["Broken Fan"] = "Sucata", -- Ventilador Quebrado
    ["Old Radio"] = "Sucata", -- Rádio Velho
    ["Broken Microwave"] = "Sucata", -- Microondas Quebrada
    Tyre = "Sucata", -- Pneu
    ["Metal Chair"] = "Sucata",
    ["Old Car Engine"] = "Sucata", -- Motor de Carro Velho
    ["Washing Machine"] = "Sucata", -- Máquina de Lavar Roupa
    ...
    Shield = "Armas e Armaduras", Lantern = "Armas e Armaduras", -- Lanterna
    Compass = "Armas e Armaduras", Map = "Armas e Armaduras",
    ...
    Flower = "Outros",
    Feather = "Outros", -- Pena
    Fire = "Outros", -- Incêndio (se for um item)
    ["Sacrifice Totem"] = "Outros", ["Old Rod"] = "Outros", ["Coin Pile"] = "Outros",
    ["Infernal Bag"] = "Outros", ["Giant Bag"] = "Outros", ["Good Bag"] = "Outros",
    ["Old Lantern"] = "Outros", ["Strong Lantern"] = "Outros", Diamond = "Outros",
    ["Defense Sketch"] = "Outros",
...
-- Função auxiliar para substituir table.find (movida para cá)
...
-- Expandido a lista de nomes do Workbench
...
local function applyPlayerBringState(disconnectCamera)
    ...
    -- Salva o estado original (apenas na primeira vez, ou se o estado for 'normal' antes de aplicar)
    -- Isso garante que, mesmo que você ative o Untouchable/Freecam um por vez, os valores originais sejam do estado "normal".
    if not bringUntouchable and not bringFreecamActive then 
    ...
    elseif bringUntouchable then
        ...
        restorePlayerState() -- Caso os toggles sejam desativados enquanto ativo, restaura.
    end
end
...
local function runBringLogic(categoryName, specificItemNames, bringRange, maxQuantity, bringCooldown, bringMethod)
    ...
    if not targetCFrame then 
        WindUI:Notify({Title = "Bring Manual", Content = "Destino '" .. selectedBringDestination .. "' nao encontrado ou invalido!", Color = "Red", Duration = 3})
    ...
    if bringMethod == "Simples" then
        ...
        while itemsProcessedThisFrame < currentMaxQuantity and categoryControl.lastIndex <= #itemsFoundInRange do
            local itemInstance = itemsFoundInRange[categoryControl.lastIndex]
            if itemInstance and itemInstance.Parent then
                ...
                if targetPart and targetPart.Parent and targetPart:IsA("BasePart") then
                    ...
                    task.wait(0.1) -- COOLDOWN MÍNIMO PARA CADA ITEM CAIR
                end
            end
            categoryControl.lastIndex = categoryControl.lastIndex + 1
        end

        if categoryControl.lastIndex > #itemsFoundInRange then
            categoryControl.lastIndex = 1
        end
    ...
    elseif bringMethod == "Teleporte" then
        ...
        if itemToBring and itemToBring.Parent then
            ...
            -- Aplica o estado do jogador antes do TP
            ...
            trackedGroundItems[itemToBring] = nil
            task.wait(0.05)
            ...
            -- Restaura o estado do jogador após o TP do item
            restorePlayerState()
...
local function bringItemsOnceForCategory(categoryName)
    ...
    -- Aplica o estado do jogador antes de iniciar o bring
    applyPlayerBringState(true)
    
    local success = runBringLogic(categoryName, control.specificItem, bringRange, maxQuantity, cooldown, currentBringMethod)

    -- Restaura o estado do jogador SEMPRE após a tentativa de puxar
    restorePlayerState()
...
-- A função toggleBringForCategory não será mais usada para os botões de categoria
-- mas é mantida caso algum outro toggle (não implementado aqui) precise dela.
local function toggleBringForCategory(categoryName, state)
    local control = CATEGORY_CONTROLS[categoryName]
    if not control then return end

    if state then
        control.active = true
        WindUI:Notify({Title = "Bring Automatico", Content = "Bring de " .. categoryName .. " ATIVADO (contínuo)! (OBS: As categorias de item agora são botões)", Color = "Green", Duration = 2})
    else
        if control.connection then
            control.connection:Disconnect()
            control.connection = nil
        end
        control.active = false
        WindUI:Notify({Title = "Bring Automatico", Content = "Bring de " .. categoryName .. " DESATIVADO! (contínuo)", Color = "Gray", Duration = 2})
    end
end
...
for _, categoryName in ipairs(BRINGABLE_CATEGORIES_LIST) do
    if categoryName == "Meteorito" then TabBring:Section({ Title = "Trazer Itens de Meteorito" }) end
    if categoryName == "Cultista" then TabBring:Section({ Title = "Trazer Itens Cultistas" }) end
    if categoryName == "Armas e Armaduras" then TabBring:Section({ Title = "Trazer Armas e Armaduras" }) end
...
stopBringButton.MouseButton1Click:Connect(function()
    restorePlayerState() -- Garante que o estado do player seja restaurado ao clicar no pânico
    stopBringButton.Visible = false -- Oculta-se após restaurar o estado
    WindUI:Notify({Title = "Pânico", Content = "Estado do jogador restaurado!", Color = "Red", Duration = 3})
end)

-- Funcao para mostrar/esconder o botao de panico
-- (Mantido para compatibilidade, mas o botão de bring não o tornará visível diretamente agora)
local function updateStopButtonVisibility()
    -- Este botão não será automaticamente visível pelos "Bring" de categoria (que agora são de uma vez).
    -- Ele só será visível se houver alguma funcionalidade *contínua* que o ative explicitamente.
...
local function onCharacterAdded(char)
    ...
    if humanoid and humanoid.Parent then
        ...
        originalPlayerCanCollide = (hrp and hrp.CanCollide) or true
